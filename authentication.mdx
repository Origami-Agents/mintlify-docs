...
Requests from organizations on a free plan return `402 Payment Required` with
`code: "SUBSCRIPTION_REQUIRED"` — upgrade to a paid plan to use the API.
...
Every API key in your organization shares the same quota. Additional per-IP
limits may apply to protect against abuse.
...
When rate-limited, the API returns `429 Too Many Requests`. Every response
includes headers so you can track usage proactively:

| Header | Description |
|--------|-------------|
| `X-RateLimit-Limit-Global` | Maximum requests allowed in the window |
| `X-RateLimit-Remaining-Global` | Requests remaining in the current window |
| `X-RateLimit-Reset-Global` | Unix timestamp when the window resets |
| `X-RateLimit-Limit-Insert` | Maximum insert requests allowed in the window (write routes only) |
| `X-RateLimit-Remaining-Insert` | Insert requests remaining in the current window |
| `X-RateLimit-Reset-Insert` | Unix timestamp when the window resets |
