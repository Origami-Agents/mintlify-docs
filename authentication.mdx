...
All endpoints are rate-limited per organization:

| Scope | Limit |
|-------|-------|
| All endpoints | 100 requests / minute |
| `POST /tables/{tableId}/rows` | 10 requests / minute |

When rate-limited, the API returns `429 Too Many Requests` with headers indicating
when you can retry:
