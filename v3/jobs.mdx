...
Every async v3 operation returns the same **Job**. Searches, fetches, enrichment,
campaign drafts, domain purchases, mailbox provisioning, and chat messages all
share this object.
...
`succeeded` is the important difference from v2. A v2 run could `completed`
while cells were still filling in. A v3 Job does not.
...
header. Honor them. Polling faster does not finish the Job sooner — those reads
