...
<ResponseField name="Enrichment run" type="object">
  ...
  For `enrich=true` batches the object also carries a `tableRunId` — the id of
  the parent `table_run`. Use it with
  [`GET /tables/{tableId}/runs/{runId}`](/agents/reference/get-table-run)
  to read the parent's status, timestamps, and terminal child counts (the
  durable recovery read if a `table.run.completed` webhook is missed).
  `tableRunId` is `null` for `enrich=false` write batches and pre-migration batches.
...
<ResponseField name="Table run" type="object">
  The durable parent of a table operation (e.g. an API enrichment batch). Created
  automatically alongside each `enrich=true` upsert. Carries `source`, `status`,
  timestamps, an optional `failureReason`, and terminal child-outcome `counts` once
  complete. This is the recovery read if you miss a `table.run.completed` webhook.
  Endpoint: [`GET /tables/{tableId}/runs/{runId}`](/agents/reference/get-table-run).
</ResponseField>
